...
# The PTF model constructed based on Eq 3.9 in the paper Temporal Collaborative Filtering with Bayesian Probabilistic Tensor Factorization.
class PTF():
...
class Mean() :
    ...
    # Calculate MAE for the mean method.
    def MAE (self, i, j, k):
...
def main():
    ...
    # Build a random forest regressor using the same data sets as in CART.
    rf = RF(cart.X, cart.y)

    # Build a multi-layer perceptron regressor using the same data sets as in CART.
    ...
    # Visualize the MAEs for different models.
